...
import com.telegram.bot.fileloadingmanagers.ConversionInfo;
import com.telegram.bot.fileloadingmanagers.FileLoadingManager;
import com.telegram.bot.fileloadingmanagers.TelegramFileLoadingManager;
import com.telegram.bot.handlers.scripts.helperclasses.DataDownloader;
import com.telegram.bot.handlers.scripts.helperclasses.FixedSizeList;
import com.telegram.bot.handlers.scripts.helperclasses.ReplyKeyboards;
import com.telegram.bot.handlers.scripts.helperclasses.ScriptStage;
...
import com.telegram.convertations.converters.imgconverters.factory.AbstractImgConverterFactory;
import com.telegram.convertations.converters.imgconverters.factory.ImgConverterFactory;
        ...
        if (update.hasCallbackQuery()) {
            ...
            if (stage.isLoadingFile()) {
                if (message.hasText()) {
                    ...
                    if (!addDocumentWithFailMessage(photoSizeToDocument(photo), chatId)) {
                        convertAndUploadImages();
                        stop();
                    }
                    LOG.debug("[{}] Photo {} has been downloaded. Current fileId is {}.",
                            chatId,photo.getFileUniqueId(), photo.getFileId());
                } else {
